...
var textbox2 = dl.DLTextbox();
textbox2.xy = new dl.XYInfo(3,3,50,18);
textbox2.fg_color = new dl.DLColor(43,89,249);
textbox2.bg_color = new dl.DLColor(0,0,0);
textbox2.border_color = new dl.DLColor(0, 200, 0, 80);
textbox2.border_color.set_intensity (80);
textbox2.border_width = 1;
textbox2.scroll_type = 3; //SCROLL_V
textbox2.char_buffer_size = 0;
textbox2.control = 12;
...
exports.textBox = function() {
  console.log ("textbox");
  textbox2.is_final=1;
  result = textbox2.BuildMessage ();
  var send_buf = result.result_buffer.slice(0,result.result_bytes);
  display.send(send_buf);
};

exports.textBoxMove = function(direction) {
  console.log ("textbox");
//  textbox2.is_final=1;
  if (direction == "RIGHT") {
    if (textbox2.xy.x < 120) {
      textbox2.xy.x++;
    }
  }
  if (direction == "LEFT") {
...
exports.originalTest = function() {
  ...
  send_buf = result.result_buffer.slice(0,result.result_bytes);
  display.send(send_buf, reportStatus);
